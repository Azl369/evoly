...
项目已进入 V0.4-alpha 阶段，当前主线是在 V0.3 文档库基础上，补齐全局主题/UI 体验，推进 Windows 双形态提醒模式，并接入 Android + Windows 的 Supabase 账号同步最小闭环。
...
- 文档库已升级为“目标档案夹”结构：一个目标天然对应一个文件夹，文件夹内承载过程记录、项目总结、复盘和知识笔记。
- 目标详情页支持打开目标档案夹，并在目标档案夹内查看全部关联文档、新建过程记录和创建项目总结草稿。
- 文档自动保存和目标关联文档闭环已接入，后续继续补任务链接、文件夹内筛选和 Markdown 导出。

V0.4-alpha 当前重点：

- 全局主题系统：支持“星轨蓝 / 极光绿 / 暮光橙 / 石墨 HUD”四套 HUD 主题，以及跟随系统、浅色、深色模式。
- 全局 UI 重构：补齐设计 token、共享组件、紧凑表单弹层、任务/目标卡片、Today Coach、统计图和文档库视觉统一。
- Windows 双形态提醒：支持完整工作台、右上角 HUD 迷你面板、托盘隐藏、任务点击回到编辑闭环和桌面模式设置。
...
- **主题与设计系统**：新增 HUD 主题预设、SQLite 设置持久化、`EvolyDesignTokens`、共享 glass surface 组件，并统一主要 Material 3 组件主题。
- **页面 UI 升级**：Today、Goals、Goal Detail、Task Sheets、Stats、Documents、Settings/Review 已按更轻量、更高级、更紧凑的方向重构。
- **Windows 双形态**：新增桌面窗口控制层、托盘菜单、透明 HUD 迷你面板、任务打开闭环和 Windows 桌面模式设置。
...
- **开发文档**：新增主题系统计划、Windows 双形态计划、V0.4 同步计划和 Supabase `sync_schema.sql`，调试指南补充 Supabase `--dart-define` 用法。
...
- UI 策略：Material 3、HUD 主题预设、设计 token、共享 glass surface 组件、语义色
...
- App 支持四套 HUD 主题预设：星轨蓝、极光绿、暮光橙、石墨 HUD。
- App 支持跟随系统、浅色、深色三种主题模式，并持久化保存到本地 SQLite 设置表。
- 全局 UI 已接入 HUD 设计 token 和共享 glass surface 组件，主要页面的卡片、徽标、指标、空状态、弹层和列表密度保持一致。
- Windows 端支持完整工作台、右上角 HUD 迷你面板和托盘后台待命三种使用路径。
- Windows 迷你面板展示下一条提醒、今日未完成/已到时指标和高优先级 Top 3，支持完成、延后、隐藏、展开/收起和点击任务回到完整编辑。
- Windows 桌面模式支持关闭行为、托盘点击行为、迷你面板置顶和面板位置持久化设置。
