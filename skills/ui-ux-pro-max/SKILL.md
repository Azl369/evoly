...
This skill is fixed into this repository at `skills/ui-ux-pro-max/`.
Run local scripts from the project root with `python` on Windows, for example:

```powershell
python skills\ui-ux-pro-max\scripts\search.py "Flutter productivity app" --design-system -p "Evoly"
```

Additional upstream sub-skills are preserved under
`references/subskills/`. When a sub-skill example mentions
`~/.claude/skills/...` or `.claude/skills/...`, adapt that path to the local
`skills/ui-ux-pro-max/references/subskills/...` copy.
