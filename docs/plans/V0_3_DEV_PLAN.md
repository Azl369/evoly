...
- 最近更新。
- 目标详情页展示关联文档。
- Markdown 增强预览：数学公式、ChordPro 和弦谱、吉他 TAB、ABC 简谱。
...
- V0.3.4 Markdown 音乐谱块预览：
  - 接入本地包 `evoly_markdown_music_preview`。
  - Markdown 预览合并 `MarkdownMusicSupport`，支持 `chordpro`、`tab`、`abc` fenced code block。
  - 文档编辑页新增音乐谱块插入菜单，使用 `MarkdownMusicTemplates.all` 生成模板。
  - 该能力只做本地预览，不联网、不使用 WebView、不做音频播放。
